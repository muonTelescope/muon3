...
Key results include a mean detected photoelectron yield of approximately 25--40~p.e.\ per minimum-ionizing muon (after all losses), good position uniformity, and power/thermal budgets supporting portable operation. Improvements over prior stand-in models include realistic cosmic muon spectra and angular distributions, refined material and surface properties drawn from literature, and a more representative fiber-loop geometry.
...
Using the improved stand-in (anchored to the Geant4 hits.csv data set and literature parameters), we obtain:
...
Quantity & Value \\
\midrule
Mean energy deposit (MIP, 10 mm) & $\approx 2\,\mathrm{MeV}$ \\
Scintillation photons produced & $\sim$20\,000 \\
Mean detected p.e.\ (after WLS + transport + PDE) & 25--40 \\
Position uniformity (rms) & $\sim$20--25\% \\
Arrival time spread (FWHM) & 8--12~ns \\
\bottomrule
\end{tabular}
\caption{Key Geant4-derived detector metrics (stand-in + partial real data; full optical run pending complete Geant4 libraries).}
\end{table}

Light yield maps show the expected drop toward corners and edges farthest from the fiber exit, consistent with looped-fiber designs in the literature \cite{muography-ej200}.
