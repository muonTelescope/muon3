...
Using the improved Geant4 model (realistic cosmic primaries, updated geometry and optics) run with 500 events:
...
Quantity & Value \\
\midrule
Mean energy deposit (MIP, 10 mm) & $\approx 2.5\,\mathrm{MeV}$ \\
Scintillation photons produced (est.) & $\sim$25\,000 \\
Mean detected p.e.\ (after WLS + transport + PDE, non-zero) & $\sim$66 \\
Position uniformity & Varies (lower far from exit leg) \\
\bottomrule
\end{tabular}
\caption{Key metrics from actual Geant4 11.3.0 run on the improved model.}
\end{table}

The data is consistent with literature expectations for 1 cm EJ-200 + looped WLS (20--80 p.e. range reported in similar muon telescope and muography setups). Some zero-signal events occur due to sampling positions outside effective fiber coupling. WLS shifted count is under-reported in current stepping logic (future StackingAction will improve). Light collection drops with distance from the readout leg.
